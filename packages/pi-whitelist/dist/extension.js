 ...
 */
import { PermissionManager, parseRuleString, serializeRuleString, } from "./index.js";
import { expandDenyPaths, FILE_TOOLS, } from "./deny-paths.js";
...
const log = {
    info: (msg, ...args) => console.log(`[pi-whitelist] ${msg}`, ...args),
    warn: (msg, ...args) => console.warn(`[pi-whitelist] ${msg}`, ...args),
};
function uiNotify(ctx, msg, level = "info") {
    if (ctx.hasUI && ctx.ui) {
 ...
 */
async function numberedPrompt(ctx, title, options) {
    if (!ctx.hasUI)
        return 0;
...
function readPermissionSettings(filePath) {
...
function loadSettingsIntoManager(filePath, manager, source) {
    const settings = readPermissionSettings(filePath);
    if (!settings)
        return null;
    for (const rule of settings.allow)
        manager.addRule(parseRuleString(rule), "allow", source);
    for (const rule of settings.deny)
        manager.addRule(parseRuleString(rule), "deny", source);
    for (const rule of settings.ask)
        manager.addRule(parseRuleString(rule), "ask", source);
    for (const rule of expandDenyPaths(settings.denyPaths))
        manager.addRule(parseRuleString(rule), "deny", source);
    let validatedMode;
...
function persistRule(filePath, rule, behavior) {
...
function persistDefaultMode(filePath, mode) {
    const dir = filePath.substring(0, filePath.lastIndexOf("/"));
    if (!existsSync(dir))
        mkdirSync(dir, { recursive: true });
    let settings = {};
...
export default async function whitelistExtension(pi) {
    ...
    const cycleMode = (ctx) => {
        const order = ["default", "auto", "plan", "bypassPermissions"];
        const m = manager.getMode();
    ...
    pi.on("tool_call", async (event, ctx) => {
        ...
        if (gateAction.kind === "dangerous-prompt") {
            ...
            const choice = await numberedPrompt(ctx, `${dangerousLabel}\n\nDangerous command. Allow anyway?`, ["1 Allow once", "2 Allow always", "3 Deny"]);
            if (choice === 2) {
        ...
        if (suggestions.length === 0) {
            const choice = await numberedPrompt(ctx, `[auth] ${label}`, ["1 Allow once", "2 Allow always", "3 Deny"]);
            if (choice === 1) {
                // Check progressive learning
                const suggestion = recordAllowOnce(tracker, toolName, ruleContent);
                if (suggestion) {
                    const autoAccept = await ctx.ui.confirm(`[suggest] You've allowed ${suggestion.count}x similar commands. Allow ${suggestion.rule} always?`, "Allow prefix");
                    if (autoAccept) {
                        manager.addRule(parseRuleString(suggestion.rule), "allow", "localSettings");
                        try {
                            persistRule(localSettingsPath, parseRuleString(suggestion.rule), "allow");
                        }
                        catch (err) {
                            log.warn(`Failed to persist: ${err}`);
                        }
        ...
        if (choice === 2) {
            ...
            try {
                persistRule(localSettingsPath, { toolName, ruleContent: specificSuggestion.pattern }, "allow");
            }
            catch (err) {
                log.warn(`Failed to persist: ${err}`);
            }
        ...
        if (choice === 3 && hasTwoScopes) {
            ...
            try {
                persistRule(localSettingsPath, { toolName, ruleContent: broadSuggestion.pattern }, "allow");
            }
            catch (err) {
                log.warn(`Failed to persist: ${err}`);
            }
        ...
        handler: async (args, ctx) => {
            ...
            if (subcommand === "status") {
                ...
                if (allRules.length === 0) {
                    uiNotify(ctx, `Whitelist: (no rules)\n\nMode: ${manager.getMode()}\n\n/whitelist allow|deny|deny-path|mode`, "info");
                }
                else {
                    const lines = allRules.map((r) => `  ${r.ruleBehavior}: ${r.ruleValue.toolName}${r.ruleValue.ruleContent ? `(${r.ruleValue.ruleContent})` : ""} [${r.source}]`);
                    uiNotify(ctx, `Whitelist (${allRules.length}):\n${lines.join("\n")}\n\nMode: ${manager.getMode()}`, "info");
                }
                return;
            }
            if (subcommand === "allow" || subcommand === "deny") {
                const behavior = subcommand;
            ...
            if (subcommand === "mode") {
                ...
                if (parts.length > 3 || (parts.length === 3 && parts[2] !== "--save" && parts[2] !== "--save=true")) {
                    uiNotify(ctx, `Invalid mode command format.\nUsage: /whitelist mode <mode> [--save]`, "error");
                    return;
                }
                if (!newMode) {
                    uiNotify(ctx, `Mode: ${manager.getMode()}\nCycle: default -> auto -> plan -> bypass -> default\n\nOptions: default · auto · bypass · plan · acceptEdits · dontAsk\nUsage: /whitelist mode <mode> [--save]`, "info");
                    return;
                }
                if (newMode === "cycle") {
                ...
                if (!["default", "auto", "bypassPermissions", "plan", "acceptEdits", "dontAsk"].includes(newMode)) {
                    uiNotify(ctx, `Invalid mode: ${newMode}\n\nOptions: default · auto · bypass · plan · acceptEdits · dontAsk`, "info");
                    return;
                }
                ...
                if (isSave) {
                    try {
                        persistDefaultMode(globalSettingsPath, newMode);
            ...
            uiNotify(ctx, `Usage: /whitelist <status|allow|deny|deny-path|mode>`, "info");
        },
    });
