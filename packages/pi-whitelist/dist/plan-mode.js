...
const PACKAGE_WRITE_COMMANDS = new Map([
...
function containsUnsafeShellSyntax(command) {
    return /[<>|&`'";${}*?[\]\\]/.test(command)
        || /\n|\r/.test(command)
        || /&&|\|\|/.test(command);
}
...
function isShellWrapper(command, args) {
    return SHELL_WRAPPERS.has(command) && args.some((arg) => /^-[A-Za-z]*c[A-Za-z]*$/.test(arg));
}
function isPackageWrite(command, args) {
...
function isFindWrite(args) {
    return args.some((arg) => UNSAFE_FIND_FLAGS.has(arg.toLowerCase()));
}
function isRipgrepUnsafe(args) {
    return args.some((arg) => arg.toLowerCase() === '--pre' || arg.toLowerCase().startsWith('--pre='));
}
export function isPlanModeBashSafe(command) {
    const trimmed = command.trim();
    if (!trimmed || containsUnsafeShellSyntax(trimmed))
        return false;
    const tokens = splitCommand(trimmed);
    if (tokens.length === 0 || isEnvironmentAssignment(tokens[0]))
        return false;
    if (tokens[0].includes('/') || tokens[0].includes('\\'))
        return false;
    const commandName = tokens[0].toLowerCase();
    const args = tokens.slice(1);
    if (isShellWrapper(commandName, args))
        return false;
    if (PRIVILEGED_COMMANDS.has(commandName))
        return false;
    if (PROCESS_CONTROL_COMMANDS.has(commandName))
        return false;
    if (NETWORK_COMMANDS.has(commandName))
        return false;
    if (isPackageWrite(commandName, args))
        return false;
    if (commandName === 'git') {
        if (isGitWrite(args))
            return false;
...
export function evaluatePlanMode(toolName, ruleContent) {
